...
namespace operation {
    ...
    void turnClaw() {
        ...
        RotateMotor(OUT_A, 100, 115); //120); // turn
    ...
    void calibrateSensor(int sensor, sensorColor color) {
        int val = ReadEV3ColorSensorLight(sensor, ReflectedLight);
        sensorValue[sensor][static_cast<int>(color)] = val;
    }

    int getSensorThreshold(int sensor, sensorColor col1, sensorColor col2, double percentageCol1) {
        int val1 = sensorValue[sensor][static_cast<int>(col1)];
        int val2 = sensorValue[sensor][static_cast<int>(col2)];
        return std::round((double)(val1 * percetageCol1 + val2 * (100 - percentageCol1))/100.0);
    }

    int getSensorThreshold(int sensor, sensorColor col1, sensorColor col2) {
        return getSensorThreshold(sensor, col1, col2, 50);
    }

    void linesquare(int sensor1, int sensor2, int sensitivity = 5) {
        while (true) {
            int val1 = ReadEV3ColorSensorLight(sensor1, ReflectedLight);
            int val2 = ReadEV3ColorSensorLight(sensor2, ReflectedLight);
            int diff = val1 - val2;

            if (abs(diff) < sensitivity) break;
            if (diff > 0) {
                OnFwdReg(OUT_B, -15);
                OnFwdReg(OUT_C, -12);
            } else {
...
namespace solution {
    ...
    void s01_scanBlocks() {
        ...
        operation::calibrateSensor(IN_2, operation::sensorColor::WHITE); // perfect location to calibrate
        operation::calibrateSensor(IN_3, operation::sensorColor::WHITE); // perfect location to calibrate
    ...
    void s04_depositSet2() {
        { // MARK: adjust robot to face east at intersection
            ...
            OnFwdReg(OUT_BC, -20);
            while (ReadEV3ColorSensorLight(IN_3, ReflectedLight) > 10) Wait(1); // move until y-1
            Off(OUT_BC);
        ...
        { // MARK: travel to yellow    
            ...
            while (ReadEV3ColorSensorLight(IN_2, ReflectedLight) > 10) Wait(1); // move to intersection
            Off(OUT_BC);
        ...
        { // MARK: go to y+1 facing east
            ...
            { // MARK: adjust robot to face east at intersection
                OnFwdReg(OUT_BC, -20);
                while (ReadEV3ColorSensorLight(IN_2, ReflectedLight) > 10) Wait(1); // move until y+1
                Off(OUT_BC);
    ...
    void s05_depositSet3() {
        { // MARK: grab the cable and position for node grabbing
            ...
            { // reach intersection
                OnFwdReg(OUT_BC, -20);
                while (ReadEV3ColorSensorLight(IN_2, ReflectedLight) > 10) Wait(1);
                Off(OUT_BC);
            }
        ...
        for (int i = 2; i > 0; --i) { // MARK: grab the first available node
            { // PID to next intersection
                double Ku = 0.5, Tu = 0.78;
                PID pid = PID(0.001, 30, -30, 0.6*Ku, 1.2*Ku/Tu, 3.0*Ku*Tu/40.0);    // Classic
                while (ReadEV3ColorSensorLight(IN_3, ReflectedLight) > 10) { // get to the next line
                    int val = ReadEV3ColorSensorLight(IN_4, ReflectedLight);
                    double ret = pid.calculate(50, val) * 1;
